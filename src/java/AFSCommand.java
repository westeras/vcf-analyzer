...
public class AFSCommand extends Command{
	...
	public AFSCommand(String VCFName, String options) throws ClassNotFoundException, SQLException{
	...
	private ArrayList<String> getVcfIDs() throws SQLException, ClassNotFoundException{
		String sql = "Select `VcfId` from `Vcf` where `VcfName`='"+this.VCFName+"'";
		ResultSet vcf= this.conn.executeQuery(sql);
		return convertToArrayList(vcf);
		
	}
	private ArrayList<String> getEntryIDs(String VcfId) throws SQLException, ClassNotFoundException {
		String sql= "Select `EntryId` from `VcfEntry` where `VcfId`= '"+VcfId+"'";
	...
	private void updateSpectra(String allele1, String allele2, String allele3) {
		...
		if (allele1==null){
			...
			System.out.println("found first allele");
			this.spectra[0]++;
		}
		if (allele2==null){
			return;
			...
			System.out.println("found second allele");
			this.spectra[1]++;
		}
		if (allele3==null){
			return;
			...
			System.out.println("found third allele");
			this.spectra[2]++;
		}
	}
